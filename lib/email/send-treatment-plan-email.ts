...
export type SendTreatmentPlanEmailResult =
  | { sent: true }
  | { sent: false; error: string }

export async function sendTreatmentPlanEmail({
  to,
  subject,
  htmlBody,
  textBody,
  ...
  pdfBuffer: Buffer
  filename: string
}): Promise<SendTreatmentPlanEmailResult> {
  const email = to.trim()
  if (!email) {
  ...
  try {
    ...
    console.error("Failed to send treatment plan email:", err)
    return {
      sent: false,
